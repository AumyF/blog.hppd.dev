...
[devcontainer.jsonの公式リファレンス](https://code.visualstudio.com/docs/remote/devcontainerjson-reference)貼っときますね (詳しくはこれ見て)。

# まとめ

WSL2だらだ適当に書くとWSLについてダラダラ書いてしまう。免罪符として「雑度が高い」タグを生み出した。こいつの存在自体の雑度が高くないか？

WSLに対してDockerの知識が全然ないことがバレたので勉強する。コンテナとイメージの違いも正直あんまりわかってない。めざせDockerの話するとめっちゃ早口になるオタク。
